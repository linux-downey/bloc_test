...
### 可变参数函数原理
在上面提到了，在参数压栈的过程中，是从右到左的顺序，即最后一个参数最先压栈，这样的压栈方式为可变参数函数的实现提供了条件。  
...
在这里，我们需要关注几个问题：
1. 为什么压栈过程需要是从右往左的顺序才方便实现可变参数传递？
2. printf()函数和上述的add()实现都在可变参数前至少提供了一个具体参数，可不可以省略这个参数呢？
3. 在使用va_arg()取出函数的值时需要指定类型，如果指定一个错误的类型会怎么样呢？
  
第一和第二个问题其实可以同时来解释，我们假设参数压栈顺序是从左至右的，又会发生什么呢？  
从上面提到的函数调用的栈过程可以知道，函数将参数压栈，接着是返回值，接着是被调用函数的局部变量等等，如果是从左至右的参数压栈顺序，CPU可以确定第一个压栈参数的地址，但是无法确认最后一个参数的地址，如果要定位最后一个参数地址，将增加系统的负担。
