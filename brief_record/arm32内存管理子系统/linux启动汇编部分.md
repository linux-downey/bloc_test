...
r1 = machine nr(自定义,imx6 上是 0xffffffff), r2 = dtb pointer.
...
如果采用单层的段(1M)映射，32位处理器最大寻址地址4GB，所以需要有4096个页表项，每个表项大小4bytes，则总共需要16KB地址。内核启动段映射页表一般在0xc0004000~0xc0008000。
...
1:	orr	r3, r7, r5, lsl #SECTION_SHIFT	@ flags + kernel base   // r3 = 0x10100c0e
	...
	// 这是映射的内核页表，属于物理地址,可以从这里看出，这里映射的是 1M 的 section，而不是 4K 的页面，可能是因为映射 4K 的页面需要的内存太多，页表项只有几十个字节(实际上整个4G空间需要16K)就好了，而且只用到了一级页表项。
