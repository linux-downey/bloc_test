...
同时，对于内核而言，动态库的加载位置是唯一的，但是对于不同的进程而言，它可以被映射到不同的虚拟地址上，当一个程序启动时，加载器会解析其 elf 文件中对应的段，如果发现需要加载对应的动态库，首先会检查是否系统中已经加载该动态库，如果已经加载，仅仅是为动态库建立虚拟地址映射，如果没有加载，就需要在内核中通过动态链接器将对应的动态库加载到内核中，然后建立虚拟地址映射。
...
无论是变量还是指令的引用，都可以使用 GOT 表的形式来实现。  
