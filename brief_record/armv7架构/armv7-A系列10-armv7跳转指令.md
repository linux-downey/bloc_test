...
### 基于 PC 的跳转
PC 指针的跳转原本是非常简单的，往 PC 中写一个特定的地址值，程序就会跳转到对应的地址处执行，这样的操作也是支持的，但是在很多情况下，是基于当前 PC 指针的偏移地址进行跳转，这个问题就要稍微复杂一些。  
...
#### PC 指针的跳转
PC 属于 arm 核心寄存器中的特殊寄存器，但是它本质上还是一个核心寄存器，所以，大多数针对核心寄存器的操作指令都可以对 PC 指针进行操作，而修改 PC 指针就是执行程序的跳转。比如 mov，add，ldr，pop等指令。    

写入 PC 的值将会被强制对齐，对齐的字节数根据对应的指令集而定，thumb下是半字，arm下是字，也就是最低位(或两位)将会被跳转指令忽略。但是可以作为其它作用，我们将在后面讨论。  
