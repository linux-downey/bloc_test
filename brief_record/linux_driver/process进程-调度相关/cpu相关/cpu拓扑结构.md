...
## 超线程
在 CPU 中,出于性能考虑,引入了流水线,超标量技术,尤其是对于超标量的引入,同时也增加了大量的硬件资源,当然,在理想状态下,这些硬件资源都应该被合理利用,但是由于指令间的依赖,指令跳转等问题,这些资源经常被闲置,为了把这些资源给充分地利用起来,干脆使用这些资源组合成一个新的逻辑 CPU 核,这也就是超线程的由来,这是超线程的由来,实际的实现中,还涉及到一些其它硬件上的修改,毕竟超线程是实现一个完整的逻辑 CPU 核.  
...
理想是美好的,现实总是比较残酷,无论是对于标量流水线还是超标量流水线,都存在一些难缠的竞争问题:
* 可能出现一个硬件部件同时被两个指令的不同阶段争用,比如指令 1 的回写操作到 CPU 寄存器组,而指令 3 的译码操作同样需要操作 CPU 寄存器组.由于这种硬件结构上的冲突并不是流水线的每个步骤都会产生,只是在特定的步骤才会产生争用,比如对于 5 级流水线而言,取指用到指令寄存器,译码阶段可能使用独立的译码器或者寄存器组,执行阶段使用 ALU,访存需要用到存储器,而回写需要用到存储器或者寄存器组.通常是硬件上对于可能发生的争用进行处理,比如快速器件在一个周期内的多次操作(时钟由执行最慢的组件决定),或者设置特定器件的旁路,甚至直接采用效率低下的流水线停顿方式.   
* 上文的讨论中没有考虑到指令之间的关联性,实际上大部分指令都是存在关联的,这就可能导致一些问题,比如这两条简单的指令:{a=a+1,b=a+1},按理说,b 的执行需要 a 的结果,指令也是这么执行的,但是在流水线中,可能第一条指令刚执行到访存,并没有回写,而第二条指令已经到执行阶段,需要第一条指令的执行结果,这种数据上的依赖是非常常见的,解决这种方式最简单的方式就是万能的流水线停顿,就是让下一条操作直接等着,等到第一条指令执行完,这个方式足够简单稳定但是效率低下
